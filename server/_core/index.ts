...
import { registerLocalAuthRoutes } from "./localAuth.ts";
...
async function startServer() {
  ...
  const runReminderJob = async () => {
    ...
    try {
      ...
      if (result.sent > 0) {
        console.log(`[Reminders] Sent ${result.sent} reminder(s) after checking ${result.checked} booking(s)`);
      }
    } catch (error) {
  ...
  // Local auth (dev only / controlled env)
  ...
  app.post("/api/webhooks/mercado-pago", async (req, res) => {
    try {
      ...
      await handleMercadoPagoWebhook(req.body, { slug });
