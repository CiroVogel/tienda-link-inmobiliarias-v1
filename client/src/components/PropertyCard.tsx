...
export function PropertyCard({ property, slug }: PropertyCardProps) {
  ...
  return (
    <article className="group overflow-hidden rounded-2xl border border-zinc-100 bg-white shadow-sm transition hover:shadow-md">
      <Link href={`/${slug}/propiedades/${property.id}`}>
        <div className="relative aspect-[4/3] overflow-hidden bg-zinc-100">
          ...
          <div className="absolute left-3 top-3 flex flex-wrap gap-2">
            <span className="rounded-full bg-white px-3 py-1 text-[11px] font-black uppercase tracking-[0.14em] text-zinc-950 shadow-sm">
              {getOperationLabel(property.operation)}
            </span>

            {property.status !== "available" ? (
              <span className="rounded-full bg-[#1A2744] px-3 py-1 text-[11px] font-black uppercase tracking-[0.14em] text-white shadow-sm">
      ...
      <div className="p-5">
        <div className="mb-4 flex items-start justify-between gap-4">
          <div>
            ...
            <Link href={`/${slug}/propiedades/${property.id}`}>
              <h3 className="text-xl font-black leading-tight tracking-tight text-zinc-950 transition group-hover:text-[#1A2744]">
                {property.title}
              </h3>
            </Link>
          </div>
        ...
        <div className="flex items-center justify-between gap-3 border-t border-zinc-100 pt-4">
          ...
          {requestable ? (
            <Link href={`/${slug}/solicitar-visita/${property.id}`}>
              <span className="rounded-xl bg-[#1A2744] px-3 py-2 text-xs font-black uppercase tracking-[0.14em] text-white transition hover:bg-[#142039]">
                Solicitar visita
              </span>
            </Link>
          ) : (
