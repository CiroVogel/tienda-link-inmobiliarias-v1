      ...
      <div className="mx-auto max-w-6xl p-6">
        <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            {INTERESTED_STATUS_OPTIONS.map((option) => (
              <span
                key={option.value}
                className="rounded-full border border-[#ded8cc] bg-white px-3 py-2 text-xs font-bold uppercase tracking-[0.14em] text-[#465153]"
              >
        ...
        <div className="mb-6 rounded-2xl border border-[#ded8cc] bg-white p-4 shadow-sm">
          <div className="mb-4 flex gap-2 overflow-x-auto pb-1">
            ...
            >
              Todos
              <span className={`ml-2 rounded-full px-1.5 py-0.5 text-xs ${filter === "all" ? "bg-white/15" : "bg-[#f0ede6] text-[#465153]"}`}>
                {counts.all}
              </span>
            </button>
            ...
            {INTERESTED_STATUS_OPTIONS.map((option) => {
              ...
              return (
                ...
                >
                  {option.label}
                  <span className={`ml-2 rounded-full px-1.5 py-0.5 text-xs ${isSelected ? "bg-white/15" : "bg-[#f0ede6] text-[#465153]"}`}>
                    {counts[option.value]}
                  </span>
                </button>
              );
                ...
                >
                  <div className="flex flex-col gap-4 xl:flex-row xl:items-start">
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="line-clamp-2 text-sm leading-6 text-[#465153]">
                        {request.message}
                      </p>
                    </div>
                    ...
                    <div className="flex shrink-0 flex-col gap-2 xl:w-52">
                      ...
                      >
                        <SelectTrigger className="h-9 border-[#ded8cc] bg-white text-xs">
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                          {INTERESTED_STATUS_OPTIONS.map((option) => (
