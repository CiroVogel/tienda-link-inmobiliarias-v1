      ...
      <div className="mx-auto max-w-6xl p-6">
        <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            <span className="rounded-full border border-[#ded8cc] bg-white px-3 py-2 text-xs font-bold uppercase tracking-[0.14em] text-[#465153]">
              Activas: {counts.all}
            </span>
            <span className="rounded-full border border-[#ded8cc] bg-white px-3 py-2 text-xs font-bold uppercase tracking-[0.14em] text-[#465153]">
        ...
        <div className="mb-6 rounded-2xl border border-[#ded8cc] bg-white p-4 shadow-sm">
          <div className="mb-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
            {SAVED_SEARCH_STATUS_OPTIONS.map((option) => (
              <div
                ...
                className="rounded-xl border border-[#ded8cc] bg-[#f7f5ef] px-4 py-3"
              >
                <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[#465153]">
                  {option.label}
                </p>
                <p className="mt-2 text-2xl font-black tracking-tight text-[#172124]">
                  {counts[option.value]}
                </p>
              </div>
            ))}
          ...
          <div className="mb-4 flex flex-wrap gap-2">
            ...
            >
              ...
              <span
                className={`ml-2 rounded-full px-1.5 py-0.5 text-xs ${
                  filter === "all" ? "bg-white/15" : "bg-[#f0ede6] text-[#465153]"
                }`}
              >
                {counts.all}
              </span>
            ...
            >
              ...
              <span
                className={`ml-2 rounded-full px-1.5 py-0.5 text-xs ${
                  filter === "archived" ? "bg-white/15" : "bg-[#f0ede6] text-[#465153]"
                }`}
              >
                {counts.archived}
              </span>
        ...
        {isLoading ? (
          ...
          <div className="rounded-xl border border-[#ded8cc] bg-white px-6 py-16 text-center">
            <p className="font-semibold text-[#465153]">
              {filter === "archived"
              ...
              return (
                <article
                  ...
                  className={`rounded-xl border bg-white p-5 transition-shadow hover:shadow-sm ${
                    item.isArchived ? "border-[#c8c0b4] opacity-90" : "border-[#ded8cc]"
                  }`}
                >
                  <div className="flex flex-col gap-4 xl:flex-row xl:items-start">
                    <div className="min-w-0 flex-1">
                      <div className="mb-2 flex flex-wrap items-center gap-3">
                        <h2 className="truncate text-base font-black text-[#172124]">
                          {item.name}
                        </h2>
                        <span
                      ...
                      <div className="grid gap-3 md:grid-cols-2">
                        <div className="rounded-xl border border-[#ded8cc] bg-[#f7f5ef] px-3 py-3">
                          <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[#465153]">
                            Búsqueda
                          </p>
                          <p className="mt-1 text-sm text-[#172124]">
                        ...
                        <div className="rounded-xl border border-[#ded8cc] bg-[#f7f5ef] px-3 py-3">
                          ...
                          <p className="mt-1 text-sm text-[#172124]">{item.budget}</p>
                        </div>
                      </div>

                      {item.comments ? (
                        <div className="mt-3 rounded-xl border border-[#ded8cc] bg-[#f7f5ef] px-3 py-3">
                          <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[#465153]">
                            Comentarios
                          </p>
                          <p className="mt-1 text-sm leading-6 text-[#465153]">{item.comments}</p>
                        </div>
                      ) : null}

                      <div className="mt-3 rounded-xl border border-[#ded8cc] bg-white px-4 py-4">
                        <div className="mb-3 flex items-center gap-2">
                          <StickyNote className="h-4 w-4 text-[#465153]" />
                          <p className="text-xs font-bold uppercase tracking-[0.16em] text-[#465153]">
                            Notas internas
                          </p>
                        </div>
                        ...
                        {item.notes.length > 0 ? (
                          <div className="mb-3 space-y-2">
                            {item.notes.map((note) => (
                              <div
                                key={note.id}
                                className="rounded-lg border border-[#ded8cc] bg-[#f7f5ef] px-3 py-3"
                              >
                                <p className="text-sm leading-6 text-[#465153]">{note.text}</p>
                                <p className="mt-1 text-[11px] uppercase tracking-[0.12em] text-[#465153]">
                          ...
                          <p className="mb-3 text-sm text-[#465153]">
                        ...
                        <div className="grid gap-2">
                          <textarea
                            ...
                            className="resize-none rounded-lg border border-[#ded8cc] bg-white px-3 py-3 text-sm text-[#172124] outline-none placeholder:text-zinc-600 focus:border-[#12383d]"
                          />
                          <div>
                            <Button
                              type="button"
